...
void PVEGamePage(){
	...
	std::string delpath = "del gamereplay\\" ;
	std::string delpathlss;
	ExMessage m;
	FILE *fp = NULL;
	...
	std::string namep = "gamereplay\\" + (std::string)name;
	...
	while (1){			
		...
		if(nowGameIdentity == !identity){
			if (difficulty){
				game.easyAI(nowGameIdentity, gameCount);
				game.Fall();
				game.lastNotice(gameCount, nowGameIdentity); 
				fprintf(fp, "%d %d\n", game.nx, game.ny);
			}
			else {
				game.advancedAI(nowGameIdentity, gameCount);
				game.Fall();
				game.lastNotice(gameCount, nowGameIdentity); 
				fprintf(fp, "%d %d\n", game.nx, game.ny);
			}
